...
class TentativeQuizSerializer(serializers.ModelSerializer):
    ...
    score_de_reussite = serializers.IntegerField(
        source='quiz.score_de_reussite', read_only=True
    )
    ...
    nombre_bonnes_reponses = serializers.SerializerMethodField()
    nombre_mauvaises_reponses = serializers.SerializerMethodField()

    class Meta:
        model = TentativeQuiz
        fields = [
            'id', 'quiz_id', 'module_slug', 'statut', 'score',
            'score_de_reussite', 'points_obtenus', 'points_total', 'est_reussi',
            'nombre_questions', 'nombre_reponses', 'nombre_bonnes_reponses',
            'nombre_mauvaises_reponses', 'cree_le', 'soumise_le', 'reponses',
        ]
    ...
    def get_nombre_bonnes_reponses(self, obj):
        if obj.statut != TentativeQuiz.Statut.SOUMISE:
            return 0
        return obj.reponses.filter(est_correcte=True).count()

    def get_nombre_mauvaises_reponses(self, obj):
        if obj.statut != TentativeQuiz.Statut.SOUMISE:
            return 0
        return obj.reponses.filter(est_correcte=False).count()
