...
class DefiPopulaireSerializer(serializers.ModelSerializer):
    """Version publique et légère utilisée sur la page d’accueil."""

    module_slug = serializers.SlugField(source='module.slug', read_only=True, allow_null=True)
    module_titre = serializers.CharField(source='module.titre', read_only=True, allow_null=True)
    image_couverture_url = serializers.SerializerMethodField()
    nombre_participants = serializers.IntegerField(read_only=True)
    nombre_soumissions = serializers.IntegerField(read_only=True)
    nombre_validations = serializers.IntegerField(read_only=True)
    score_popularite = serializers.IntegerField(read_only=True)

    class Meta:
        model = Defi
        fields = [
            'id', 'module_slug', 'module_titre', 'titre', 'description',
            'image_couverture_url', 'niveau', 'duree_estimee',
            'points_recompense', 'nombre_participants', 'nombre_soumissions',
            'nombre_validations', 'score_popularite',
        ]
