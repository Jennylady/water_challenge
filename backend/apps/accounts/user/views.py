...
from .utils import ActivationTokenError, activate_account_email, send_account_activation_email
...
class ActivateEmailAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            _, activated = activate_account_email(serializer.validated_data['token'])
            message = (
                'Email confirmé avec succès. Vous pouvez maintenant vous connecter.'
                if activated
                else 'Cette adresse email est déjà confirmée.'
            )
            ...
            return Response(
                {
                    'success': False,
                    'message': "Erreur interne lors de la confirmation de l'email.",
...
class ResendConfirmationAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            activation_link = send_account_activation_email(serializer.user, rotate_nonce=True)
